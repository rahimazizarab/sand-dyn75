...
// $Id: tcpunix.c,v 1.6 2006-12-08 07:52:09 jj Exp $

// $Log: tcpunix.c,v $
// Revision 1.6  2006-12-08 07:52:09  jj
...
unsigned char DCCascii_to_ebcdic[] = {
...
unsigned char DCCebcdic_to_ascii[] = {
...
char    Ccom_opn [Ccom];
char    Ccom_blk [Ccom]; /* Is a blocking socket? */
SOCKET  Ccom_han [Ccom];

long    CerrGen;         /* Last error */
long    Cerr     [Ccom]; /* Last error for specific socket */

selects_ptr Cselect [Ccom];      /* Array for the 3 arrays of select input/output data */

/**********************************************************************************/

LOCK tcpip_lock;
long tcpip_init_req = 1;
...
void set_reg (long * regs, long r, long v) {
    regs [r * 2] = v;
};

long get_reg (long * regs, long r) {
    return (regs [r * 2]);
};
...
long lar_tcpip (long * regs) {
    ...
    fprintf (stderr, "***** in lar_tcpip  tcpunix.c   ******  \n");
    return;
    if (tcpip_init_req) {
        tcpip_init_req = 0;
        tcpip_init ();
...
long check_not_sock (long s, talk_ptr t) {
...
/*
        sockets.fd_count = 1;
        sockets.fd_array [0] = Ccom_han [aux1];
*/
        ...
        l = Ccom_han [m] = accept (Ccom_han [aux1], &Slocal_adx, (unsigned int *)&i);
        ...
        /* Strange how everything in this struct is network order except 'family' */
...
*/
    ...
    case 16: /* GETSOCKNAME */
        ...
        i = t->len_out;
        t->ret_cd = getsockname (Ccom_han [aux1], (struct sockaddr *)(t->buffer_out), (unsigned int *)(&i));
