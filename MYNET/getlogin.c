/*****************************************************************************
 * Example: Chat server using NetLib - gets uid of connections if possible
 * Author:  Simon Amor <simon@foobar.net>
 * Sets up a simple chat server on specified port of your machine
 ***/
...
//extern fdopen(int, char *);
extern strcasecmp(const char *, const char *);
...
/* Converts chars to upper case, toupper() macros sometimes fails */
int Toupper(c)
  int c;
{
  if (islower(c))
...
/* Filter <32 and >126, but not 13 or 10 from string */
char *filter(char *string)
{
  char *temp=malloc(strlen(string)+3);
  int loop,pos;
  ...
  {
    if ((string[loop]>31) && (string[loop]<127))
      temp[pos++]=string[loop];
  }
  temp[pos++]='\n';
  temp[pos++]='\r';
  temp[pos++]='\0';
 ...
 * process a command
 ***/
int process_command(char *buffer,int counter)
{
  ...
  if (!strcasecmp(buffer,"quit"))
  {
    ...
    sprintf(global_string,"Connection closed to line %d\n",counter+1);
    fd_array[counter]=0;
    broadcast(CONNS,(int *)&fd_array,global_string);
    return 1;
  }
  return 0;
}
...
int main()
{
  ...
  int newsocket, ok=0;
  int new_conns;
  int max_conns=sysconf(_SC_OPEN_MAX);
  int counter;
  char *uid;


  global_string=malloc(1024);

/* check buffers were allocated ok */
  if ((buffer==NULL) || (send_buffer==NULL) || (global_string==NULL))
  {
...
/* code to get remote login ID */
          if ((ok=NetGetLogin(inet_ntoa(server.sin_addr),
                        htons(server.sin_port),
                        PORT, &uid))==0)
          {
            strncpy(login[counter], uid, 9);
            strcat(login[counter], "@");
            strncat(login[counter], gethostbyaddr((char *)&server.sin_addr, 
                    sizeof(struct in_addr), AF_INET)->h_name, 20);
          }
          else
          {
            strncpy(login[counter], gethostbyaddr((char *)&server.sin_addr,
                    sizeof(struct in_addr), AF_INET)->h_name, 29);
          }
        }
      }
    }
...
/* error with receive, probably someone disconnected */
          {
            NetClose(fd_array[counter]);
...
/* receive was ok, so buffer contains input - send to all connections */
          {
            if (buffer[0]==COMCHAR)
            {
              if (process_command(buffer+1,counter)==0)
                NetPutString(fd_array[counter],"Sorry, I don't recognise that command\n\r");
            }
            else
            {
              ...
              {
                puts("malloc error");
                NetPutString(fd_array[counter],"Sorry, internal error, please repeat\n\r");
              }
              else
              {
                temp=filter(buffer);
                if (strlen(temp)>2)
                {
                  sprintf(send_buffer,"%-30s| %s", login[counter], temp);
                  broadcast(CONNS,(int *)&fd_array,send_buffer);
                } /* strlen > 0 */
                free(temp);
              } /* malloc */
