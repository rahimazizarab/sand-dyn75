...
static int   logger_init_flg = TRUE;   /* will be reset by logger_init() */
...
static char  logger_filename[MAX_PATH]= "log/hlog.txt";
...
DLL_EXPORT void logger_init(void)
{
        ...
        fileno(stderr)>=0)
    {
        ...
        else
        {
            ...
            if(!isatty(STDERR_FILENO))
            {
                ...
                {
                    printf(MSG(HHC02102, "E", "dup2()", strerror(errno)));
                    fprintf(stderr, MSG(HHC02102, "E", "dup2()", strerror(errno)));
                    exit(1);
                }
            }
        ...
        {
            logger_hrdcpyfd = 0;
            printf(MSG(HHC02102, "E", "dup()", strerror(errno)));
        ...
        if(logger_hrdcpyfd)
        {
            ...
            printf(MSG(HHC02102, "E", "fdopen()", strerror(errno)));
    ...
    if(!(logger_buffer = malloc(logger_bufsize)))
    {
        ...
        printf(MSG(HHC02102, "E", buf, strerror(errno)));
