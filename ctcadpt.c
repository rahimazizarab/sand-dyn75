...
int  CTCX_Init( DEVBLK* pDEVBLK, int argc, char *argv[] )
{
    ...
    pDEVBLK->excps = 0;
    fprintf(stderr,"CTCT initialized   \n");
    printf("CTCX_init initialized   \n");
...
static int  CTCT_Init( DEVBLK *dev, int argc, char *argv[] )
{
        ...
        lport < 1024 || lport > 65534 )
    {
        ...
        WRMSG(HHC00916, "X", SSID_TO_LCSS(dev->ssid), dev->devnum, "port number", listenp );
        printf( "PORT ==> %d \n", lport);
    ...
    if( rc < 0 )
    {
      ...
      //  printf( "I %s %s  \n ", SSID_TO_LCSS(dev->ssid), dev->devnum );
        printf( "X %s %s  \n ",  remaddr, remotep );      //BINGO GOOD ADDRESS
        WRMSG(HHC00971, "I", SSID_TO_LCSS(dev->ssid), dev->devnum, remaddr, remotep );
        WRMSG(HHC00971, "R", SSID_TO_LCSS(dev->ssid), dev->devnum, remaddr, remotep );
//  Comented out by Rahim
  ...
  //      close_socket( parm.listenfd );
        ...
        printf("ctcadpt.c  set up the listening port \n");
        memset( &(parm.addr), 0, sizeof( parm.addr ) );
                         ...
                         U16*    pResidual )
{
    ...
    if( sOffset < sizeof( CTCIHDR ) || sOffset > sCount )
    {
        ...
        printf( _("CTC101W %4.4X: Write buffer contains invalid "
                  "frame offset %u\n"),
                pDEVBLK->devnum, sOffset );
